...
def stochastic_forcing_lanczos_opt(factor = 1.0, 
                               tolerance = 1e-06, 
                               max_iter = 1000, 
                               name = '',
                               dim = None, 
  ...
  for i in range(max_iter+1):
    ...
    # eig_values_h, eig_vectors_h = np.linalg.eigh(h)

    eig_values, eig_vectors = sla.eig_banded(np.vstack((h_diag,h_sup)),lower=True)

    #print(np.linalg.norm(eig_values_h - eig_values))
    #print(np.linalg.norm(eig_vectors_h - eig_vectors))
