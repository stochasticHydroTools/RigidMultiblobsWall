...
while found_functions is False:
    try:
        from Spectral_Lub_Solver import Spectral_Lub_Solver as LS
...
if __name__ == '__main__':
    ...
    phi=0.4
    Lphi = np.sqrt(np.pi*(a**2)*2048/phi) #np.sqrt(np.pi*(a**2)*num_particles/phi)
    ...
    zMax = 6.0*a
    LSolv.Init_DPStokesSolver(z_max=zMax, r_vecs_np=None, domain='DPSC')
