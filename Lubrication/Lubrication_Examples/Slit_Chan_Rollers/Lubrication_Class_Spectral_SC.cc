...
Lubrication::Lubrication(double d_cut)
{
  ...
  std::cout << "++++++++++++++++++++++++++++\n";
  std::cout << base_dir << "\n";
  std::cout << "I'm using the new mehtod" << "\n";
  std::cout << "++++++++++++++++++++++++++++\n";
  ...
  SetMemberDataWall(base_dir+"/Resistance_Coefs/DP_case_mob_scalars_channel_H6a_Extrap_L32_eig_thresh.txt",mob_scalars_wall_2562,Wall_2562_x,false);
  SetMemberData(base_dir+"/Resistance_Coefs/FCM_res_scalars_MB_1.txt",mob_scalars_MB_11,mob_scalars_MB_12,MB_x);
  SetMemberDataWall(base_dir+"/Resistance_Coefs/DP_case_res_scalars_channel_H6a_Extrap_L32.txt",mob_scalars_wall_MB,Wall_MB_x,false);
}
...
Eigen::MatrixXd Lubrication::WallResistMatrix(double r_norm, double mob_factor[3], std::vector< double >& x, const std::vector< std::vector< double > >& vec)
{
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////////////////////////////////////////////////////////////////////
    if(epsilon > 6.0-debye_cut) // epsilon < 0.1*debye_cut
    {
      epsilon = 6.0-debye_cut;
      //tanh_fact = (1.0+tanh((epsilon+5*debye_cut)/2/debye_cut))*0.5;
      //epsilon = fmin(fabs(epsilon),debye_cut); //debye_cut;
      //if(epsilon < 0.1*debye_cut){epsilon = 0.1*debye_cut;}
      r_norm = 1.0+epsilon; //1.0+debye_cut;
    ...
    double XcPlus = Xc-4.0/3.0; 
    double YcPlus = Yc-4.0/3.0;
...
void Lubrication::ResistCOO_wall(bp::list r_vectors, double a, double eta, double wall_cutoff, bn::ndarray periodic_length, bool Sup_if_true, bp::list data, bp::list rows, bp::list cols)
{
  ...
  std::cout << "YOU HAVE SOME BULLSHIT FOR SLIT CHANNELS HARDCODED IN Lubrication::WallResistMatrixMB\n";
  std::cout << "YOU HAVE SOME BULLSHIT FOR SLIT CHANNELS HARDCODED IN Lubrication::WallResistMatrix\n";
  ...
  for(int j = 0; j < num_bodies; j++)
  {
    ...
    //std::cout << "ADD a 'continue statement on line 923 prevent the output of R wall!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
...
BOOST_PYTHON_MODULE(Lubrication_Class_Spectral_SC)
{
