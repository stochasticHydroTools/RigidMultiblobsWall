...
def blob_blob_force(r, *args, **kwargs):
  ...
  #timescale=np.int32(1)
  #if np.mod((20*time_s)//timescale, 2)==1:
  # phases=0
  #else:
   #phases=180 
  
  timescale=np.int32(1)
  if np.mod((100*time_s)//timescale, 2)==1:
    phi=0
  else:
    phi=np.pi
  Mom = np.array([np.sin(beta)*np.cos(2*np.pi*Omega*time_s), np.sin(beta)*np.sin(2*np.pi*Omega*time_s+phi), np.cos(beta)])
  
  
  # Mom = np.array([np.sin(beta)*np.sin(2*np.pi*Omega*time_s), np.sin(beta)*np.cos(2*np.pi*Omega*time_s), np.cos(beta)])
...
def calc_blob_blob_forces_python(r_vectors, *args, **kwargs):
  ...
  for j in range(Nblobs):
    s1 = r_vectors[j]
    idx = r_tree.query_ball_point(s1,r=30*(2.0*a))
