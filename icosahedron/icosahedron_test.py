...
class TestIcosahedron(unittest.TestCase):
  ...
  def test_get_icosahedron_r_vectors(self):
    ...
    location = [0., 0., 10.]
    r_vectors = ic.get_icosahedron_r_vectors(location, theta)
  ...
  def test_icosahedron_mobility_spd(self):
    ''' 
    Test that the icosahedron mobility is Symmetric and Positive Definite
  ...
  def test_icosahedron_rotation(self):
    '''Test that rotations of the icosahedron make sense.'''
    theta = np.random.normal(0., 1., 4)
    theta = Quaternion(theta/np.linalg.norm(theta))
    location = [np.random.uniform(1., 3.) for _ in range(3)]
    mobility = ic.icosahedron_mobility([location], [theta])
