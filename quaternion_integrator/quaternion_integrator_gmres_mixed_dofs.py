...
class QuaternionIntegratorGMRES(object):
  ...
  def rfd_time_step(self, dt):
	...
	if self.solver == 1:
	  ...
	  #for i in range(3):
	    #for j in range(3):
	      #if abs(self_mobility_body_tt[i][j])>1e-10:
	       #print i,j
	       #print self_mobility_body_tt[i][j]
	  #raw_input()
	elif self.solver == 2:
	  #print "=================================================== "
	...
	#raw_input()
	  ...
	  #print "==== Direct Solver  ====== "
	  #print "=================================================== " 
	  (mobility_tt, resistance_tr, res_blobs)  = self.mobility(r_vectors_ite, rotation_matrix_ite)
	  ...
	  force_deterministic = force - np.dot(resistance_tr,Omega_known)#+ force_slip
	  
	  #print "3*3 mobility_tt*6*pi*eta*Rh = "
	  #for i in range(3):
	    #for j in range(3):
	      #if abs(mobility_tt[i][j])>1e-10:
	       #print i,j
	       #print mobility_tt[i][j]*6.0*m.pi*self.eta*0.656
	  #print "6*6 resitance_tr = "
	  #for i in range(6):
	    #for j in range(6):
	      #if abs(resistance_tr[i][j])>1e-10:
	       #print i,j
	       #print resistance_tr[i][j]
	       
	  #print " np.dot(resistance_tr,Omega_known) = ",np.dot(resistance_tr,Omega_known)
	  #raw_input()
	  ...
	  velocity_deterministic = np.dot(mobility_tt, force_deterministic)
	  ...
	  velocity = velocity_deterministic # + noise_term # + divergence_term
	  ...
	  Size_system = self.Nblobs*3 + self.Nrods*3	
	  ...
	  #(sol_precond,info_precond) = spla.gmres(A,RHS,x0=self.first_guess,tol=1e-8,M=P_optim)
	  ...
	  velocity = sol_precond[self.Nblobs*3:self.Nblobs*3 + self.Nrods*3]
	...
	#print "omega = "
	  ...
	  # TO UNCOMMENT
          ...
          # TO COMMENT, ONLY FOR HYDRO TESTS
          #new_location.append(self.location[i] +  0.0087349352*np.array([0.0, 0.0, (-1.0)**(float(i+1))]))
