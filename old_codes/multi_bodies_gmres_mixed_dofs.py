...
from quaternion_integrator.quaternion_integrator_gmres_mixed_dofs import QuaternionIntegratorGMRES
...
# BLOB RADII FOR SHELL WITH Rg = 0.656
if resolution == 1: 
  A = 0.344879609550152
  Nblobs_per_rod = 12
elif resolution == 2: 
  ...
  A = 0.090496663901184 # 162 blobs
  Nblobs_per_rod = 162
elif resolution == 4: 
  A = 0.045356879822798  # 642 blobs
  Nblobs_per_rod = 642
elif resolution == 5: 
  A = 0.022692020838624
  Nblobs_per_rod = 2562 # 2562 blobs
  
# BLOB RADII FOR SHELL WITH Rg = 0.5196 <<< NEEDED TO HAVE Rh = 0.656
if resolution == 1: 
  A = 0.273171967960516
  Nblobs_per_rod = 12
...
REPULSION_STRENGTH_WALL = 20.0*0.1 * 300.*1.3806488e-5 #KT # 7.5*....
DEBYE_LENGTH_WALL = 3.0*Rh # 0.1*A

# Made these up somewhat arbitrarily
REPULSION_STRENGTH_BLOBS = 1.0*0.5 * 300.*1.3806488e-5 #KT
DEBYE_LENGTH_BLOBS = 0.5*A
...
def get_shell_initial_config(location, orientation):
  ...
  if resolution == 1:
    ...
    with open(folder_shells +'shell_3d_Nblob_12_radius_0_5196.vertex') as f:
    ...
    with open(folder_shells +'shell_3d_Nblob_42_radius_0_656.vertex') as f:
      k = 0
      for l in f:
	k = k+1
	if k>1:
    ...
    with open(folder_shells +'shell_3d_Nblob_162_radius_0_656.vertex') as f:
    ...
    with open(folder_shells +'shell_3d_Nblob_642_radius_0_656.vertex') as f:
    ...
    with open(folder_shells +'shell_3d_Nblob_2562_radius_0_656.vertex') as f:
      k = 0
      for l in f:
	k = k+1
	if k>1:
...
def create_initial_configuration_lattice(Nrods, Nx, Ny ,dx,dy,z):
  print "Nx, Ny = ", Nx, Ny
  rand_factor = 0.5*A
...
def shell_force_calculator(r_vectors):
...
def shell_force_calculator_pycuda(r_vectors):
  ...
  Nblobs_per_body = len(r_vectors[0])
  Nbody = len(r_vectors)
  force_tot = np.zeros(3*Nbody)	
  
  r_vec_for_pycuda = []
  for k in range(Nbody):
    r_vec_for_pycuda += r_vectors[k]

  repulsion = pair_int_cuda.particle_pair_interaction_pycuda(r_vec_for_pycuda,\
                                                             DEBYE_LENGTH_WALL,\
  ...
  for k in range(Nbody):
    for i in range(Nblobs_per_body):
      force_tot[3*k:3*(k+1)] = force_tot[3*k:3*(k+1)] \
	                     + repulsion[k*Nblobs_per_body*3+i*3:k*Nblobs_per_body*3+(i+1)*3]
...
def preconditioner_gmres_free_drain(vector):
  '''
  Preconditioner which solves for each body separately
  '''
 
  res = vector*(8.0*m.pi*ETA*Rh**3)                    
...
if __name__ == '__main__':
  ...
  quaternion_integrator = QuaternionIntegratorGMRES(rod_mobility,
                                               ...
                                               force_calculator=shell_force_calculator_pycuda,
  ...
  quaternion_integrator.first_guess = np.zeros(Nblobs*3 + Nrods*3)
