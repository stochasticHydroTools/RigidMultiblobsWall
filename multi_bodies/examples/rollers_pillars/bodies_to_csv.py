'''
Write blobs configuration from location and orientation to visualize with paraview

How to use:
python bodies_to_csv.py input_file name_body_ID output_name_body.config
...
if __name__ == '__main__':
  ...
  with open(config_file, 'r') as f: 
    for step in range(n_steps // n_save + 1): 
      ...
      filename = name_ID + '.csv.' + str(step)
      with open(filename, 'w') as fw:
       fw.write('xpos, ' + 'ypos, ' + 'zpos, ' +  'radius\n')
       for i in range(len(r_vectors)):
        fw.write(str(r_vectors[i,0]) + ', ' + str(r_vectors[i,1]) + ', ' + str(r_vectors[i,2]) + ', ' + str(read.blob_radius) + '\n')
