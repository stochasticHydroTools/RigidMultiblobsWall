...
def double_layer_stkfmm(r, r_null, normals, field, weights, blob_radius, PVel, L=np.zeros(3), kernel=None, *args, **kwargs):
  ...
  if build_tree:
    ...
    PVel.setup_tree(kernel)
    
  # Set double layer
  if kernel == PySTKFMM.KERNEL.PVel:
    dim_trg = 4
  elif kernel == PySTKFMM.KERNEL.PVelLaplacian:
    dim_trg = 7
  trg_value = np.zeros((N, dim_trg))
  ...
  PVel.clear_fmm(kernel)
  PVel.evaluate_fmm(kernel, np.zeros(0), trg_value, src_DL_value)  
  comm = kwargs.get('comm')
  comm.Barrier()

  # Return velocity 
  u = 2 * trg_value[:,1:4]
  if kernel == PySTKFMM.KERNEL.PVelLaplacian:
    u += (4 * blob_radius**2 / 6) * trg_value[:,4:]
    print('radius_blobs = ', blob_radius)
    print('trg_value    = ', trg_value.shape)
  return u
