...
class QuaternionIntegrator(object):
  ...
  def __init__(self, bodies, Nblobs, scheme, tolerance = None, domain = 'single_wall'): 
    ...
    self.domain = domain
    self.slip_boundary_conditions_flag = None
  ...
  def solve_mobility_problem(self, RHS = None, noise = None, noise_FT = None, AB = None, x0 = None, save_first_guess = False, PC_partial = None, *args, **kwargs): 
    ...
    # Compute variables for free slip mode
    if self.slip_boundary_conditions_flag:
      P_bodies = self.calc_P_matrix_bodies(self.bodies)
      normals = self.get_normals(self.bodies, self.Nblobs)
      Nslip = self.Nblobs
    else:
      Nslip = 0
    Bslip = np.zeros(3 * Nslip)
    
    Nconstraints = len(self.constraints)
    System_size = self.Nblobs * 3 + len(self.bodies) * 6 + Nconstraints * 3 + Nslip * 3
