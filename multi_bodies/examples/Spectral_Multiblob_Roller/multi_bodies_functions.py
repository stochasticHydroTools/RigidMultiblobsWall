...
def bodies_external_force_torque(bodies, r_vectors, *args, **kwargs):
  ...
  freq = kwargs.get('f')
  eta = kwargs.get('eta')
  ap = kwargs.get('particle_radius')
  omega = 2.0 * np.pi * freq
  for k, b in enumerate(bodies):
    force_torque[2*k,:] += -g * np.array([0., 0., 1.0])
    force_torque[2*k+1,:] += (omega*8.0*np.pi*eta*ap**3) * np.array([0., 1.0, 0.])
    # Add wall interaction
    r_b = bodies[k].location
    h = r_b[2]
    if h > ap:
      force_torque[2*k,:] += (repulsion_strength_wall / debye_length_wall) * np.exp(-(h-ap)/debye_length_wall)* np.array([0., 0., 1.0])
    else:
...
def body_body_force_torque(r, quaternion_i, quaternion_j, *args, **kwargs):
  ...
  # Get parameters from arguments
  L = kwargs.get('periodic_length')
  epss = kwargs.get('repulsion_strength')
  b = kwargs.get('debye_length')
  ...
  if r_norm > 2*ap:
    force_torque_bb[0,:] -= ((epss / b) * np.exp(-(r_norm-2*ap) / b) / np.maximum(r_norm, np.finfo(float).eps)) * r 
  else:
    force_torque_bb[0,:] -= ((epss / b) / np.maximum(r_norm, np.finfo(float).eps)) * r
...
def calc_body_body_forces_torques_python(bodies, r_vectors, *args, **kwargs):
  ...
  r_vecs_np = [b.location for b in bodies]
  r_vecs = put_r_vecs_in_periodic_box(r_vecs_np, L)
  r_tree = spatial.cKDTree(r_vecs,boxsize=L)
  
  for j in range(Nbodies):
    s1 = r_vecs[j]
    idx = r_tree.query_ball_point(s1,r=cut_pot) #2.2*ap
    idx_trim = [i for i in idx if i > j]
    for k in idx_trim:
      s2 = r_vecs[k]
      r = s2-s1
      force_torque = body_body_force_torque(r, bodies[j].orientation, bodies[k].orientation, *args, **kwargs)
      # Add forces
      force_torque_bodies[2*j] += force_torque[0]
      force_torque_bodies[2*k] -= force_torque[0]
      # Add torques
      force_torque_bodies[2*j+1] += force_torque[1]
      force_torque_bodies[2*k+1] -= force_torque[1]
