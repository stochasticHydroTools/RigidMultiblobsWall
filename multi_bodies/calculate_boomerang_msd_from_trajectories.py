...
if __name__ == '__main__':
    ...
    if data_name:
      if args.free:
        trajectory_file_names.append(
          'rod-trajectory-dt-%g-N-%s-scheme-%s-%s-%s.txt' % (
            dt, N, scheme, data_name, k))
      else:
        trajectory_file_names.append(
        'rod-trajectory-dt-%g-N-%s-scheme-%s-g-%s-%s-%s.txt' % (
            dt, N, scheme, args.gfactor, data_name, k))
    else:
      if args.free:
        trajectory_file_names.append(
          'rod-trajectory-dt-%g-N-%s-scheme-%s-%s.txt' % (
            dt, N, scheme, k))
      else:
        trajectory_file_names.append(
          'rod-trajectory-dt-%g-N-%s-scheme-%s-g-%s-%s.txt' % (
  ...
  for name in trajectory_file_names:
    ctr += 1
    data_file_name = os.path.join(DATA_DIR, 'data', name)
