...
auto createSolver(geometry geom, SpecialParameter parStar)
{
  ...
  if (geom == geometry::triply_periodic)
  {
    ...
    pse->setParametersPSE(PSE::PSEParameters{.psi = parStar.psi, .Lx = parStar.Lx, .Ly = parStar.Ly, .Lz = parStar.Lz});
    solver = pse;
  }
...
public:
  ...
  std::tuple<Vector, Vector> Solve_Mband_Sys(Vector RHS, std::vector<real> data, std::vector<int> row_idx, std::vector<int> col_idx, Matrix &U, Matrix &V, int bands)
  {
    ...
    if (ierr != 0)
    {
      std::cout << "lapack did bad" << std::endl;
    }
    ...
    for (int j = 0; j < N_blb; ++j)
    {
      ...
      if (j != (N_blb - 1))
      {
        Tens[offset + j] = RHS_p[offset + 4 * j + 3];
      }
    }
  ...
  Vector calc_D2_W(RefMatrix &T_all)
  {
    ...
    if (clamp)
    {
      std::cout << "Backward Euler not implemented yet for clamped boundary conditions\n";
      std::exit(0);
    }
